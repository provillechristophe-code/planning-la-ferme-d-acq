    ...
    <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '12px 18px', borderBottom: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', background: '#fafbfc' }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          <button style={viewBtnStyle(activeView === 'list')} onClick={() => handleViewChange('list')}>📋 Liste</button>
          <button style={viewBtnStyle(activeView === 'gantt')} onClick={() => handleViewChange('gantt')}>📊 Planning</button>
        </div>
        ...
        <button onClick={goToToday} style={{ padding: '6px 10px', borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer', fontSize: 11, fontWeight: 700, color: '#6366f1', whiteSpace: 'nowrap' }} title="Aller à aujourd'hui">📍 Aujourd'hui</button>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          <input type="date" value={dateTo} onChange={(e) => changeDateTo(e.target.value)} style={{ padding: '4px 8px', border: '1px solid #e2e8f0', borderRadius: 6, fontSize: 11, width: 120 }} />
        </div>
          ...
          {filteredBoxes.map((box) => {
            ...
            return (
              <div key={box.id} style={{ height: 48, display: 'flex', alignItems: 'center', gap: 8, padding: '0 10px', borderBottom: '1px solid #f1f5f9', background: countRes > 0 ? '#fff' : '#f8fafc' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 700, fontSize: 11, color: '#1e293b', display: 'flex', alignItems: 'center', gap: 4 }}>{box.box_number}</div>
          ...
          <div style={{ minWidth: totalDays * colWidth, position: 'relative' }}>
            {todayIndex >= 0 && (
              <div style={{ position: 'absolute', left: todayIndex * colWidth + colWidth / 2, top: 0, bottom: 0, width: 2, background: 'linear-gradient(180deg, #6366f1, transparent)', zIndex: 15, pointerEvents: 'none' }}>
                <div style={{ position: 'sticky', top: 0, background: '#6366f1', color: 'white', fontSize: 8, fontWeight: 800, padding: '3px 6px', borderRadius: '0 0 6px 6px', whiteSpace: 'nowrap' }}>📍 AUJ</div>
              </div>
            )}
              ...
              return (
                <div key={box.id} style={{ display: 'flex', position: 'relative', height: 48, borderBottom: '1px solid #f1f5f9' }}>
                  {dates.map((date) => {
                    ...
                    return (
                      <div key={date} 
                        onClick={() => { if (!occ) openNewReservation(box, date); }} 
                        onTouchEnd={(e) => { 
                          if (!occ) { 
                            e.preventDefault(); 
                            e.stopPropagation(); 
        ...
        <div style={ss.overlay} onClick={closePopup}>
          <div style={ss.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={ss.modalBody}>
              ...
              <div style={ss.grid2}>
                <div style={ss.formGroup}><label style={ss.label}>📅 Arrivée</label><input style={ss.input} type="date" value={newResForm.check_in} onChange={(e) => updateNewRes('check_in', e.target.value)} /></div>
                <div style={ss.formGroup}><label style={ss.label}>📅 Départ</label><input style={ss.input} type="date" value={newResForm.check_out} onChange={(e) => updateNewRes('check_out', e.target.value)} /></div>
              </div>
      ...
      {tooltip && (
        <div style={{
          ...
          background: '#1e293b', color: '#fff', borderRadius: 12, padding: '12px 14px',
          fontSize: 12, zIndex: 9999, pointerEvents: 'none'
        }}>
          <div><strong>{getAnimalName(tooltip.reservation.animal_id)}</strong> ({getClientName(tooltip.reservation.client_id)})</div>
          <div>📅 {displayDate(tooltip.reservation.check_in)} → {displayDate(tooltip.reservation.check_out)}</div>
        </div>
      )}
    </div>
  );
