...
var s = {
  ...
  card: { background: '#ffffff', borderRadius: 24, padding: 48, width: '100%', maxWidth: 420, boxShadow: '0 32px 64px rgba(102, 126, 234, 0.25)', position: 'relative', overflow: 'hidden' },
  title: { fontSize: 26, fontWeight: 800, color: '#1e293b', textAlign: 'center', letterSpacing: -0.5, marginBottom: 6 },
  ...
  btn: { width: '100%', background: 'linear-gradient(135deg, #6366f1, #4f46e5)', color: '#ffffff', border: 'none', padding: '16px 24px', borderRadius: 12, fontSize: 16, fontWeight: 700, cursor: 'pointer', boxShadow: '0 8px 20px rgba(99, 102, 241, 0.3)', transition: 'all 0.2s', letterSpacing: 0.2 },
...
function Login({ onLogin }) {
  ...
  var handleSubmit = function(e) {
    ...
    setError('');
    setLoading(true);
    axios.post('/api/auth/login', { email: email, password: password }).then(function(res) {
      setLoading(false);
      onLogin(res.data.token, res.data.user);
    }).catch(function(err) {
      setLoading(false);
      setError(err.response && err.response.data && err.response.data.error ? err.response.data.error : 'Erreur de connexion');
    });
  };
  return (
    <div style={s.wrapper}>
      <div style={s.card}>
        ...
        <p style={s.subtitle}>Bienvenue sur <strong>La Ferme d'Acq</strong></p>
        {error && (
        ...
        <div style={s.linkRow}>
          ...
          <a href="/register" style={s.link}>Créer un compte</a>
        </div>
        <div style={s.footer}>
