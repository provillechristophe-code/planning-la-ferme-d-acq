...
function Configuration() {
  ...
  var [configForm, setConfigForm] = useState({
    pension_name: '',
    total_boxes: 10,
    phone: '',
    address: '',
    tax_rate: 0,
    default_daily_rate: 30
  });

  var [boxForm, setBoxForm] = useState({
    box_number: '',
    box_type: 'standard',
    capacity: 1,
    daily_rate: 30
  ...
  var [batchForm, setBatchForm] = useState({
    prefix: 'A',
    separator: '-',
    startNum: 1,
    count: 5,
  ...
  var [serviceForm, setServiceForm] = useState({
    service_name: '',
    service_type: '',
    price: 0,
    description: ''
  ...
  var updateConfig = function(field, value) {
    var newForm = {};
    Object.keys(configForm).forEach(function(k) { newForm[k] = configForm[k]; });
    newForm[field] = value;
  ...
  var updateBox = function(field, value) {
    var newForm = {};
    Object.keys(boxForm).forEach(function(k) { newForm[k] = boxForm[k]; });
    newForm[field] = value;
    setBoxForm(newForm);
  };
  ...
  var updateBatch = function(field, value) {
    var newForm = {};
    Object.keys(batchForm).forEach(function(k) { newForm[k] = batchForm[k]; });
    newForm[field] = value;
  ...
  var updateService = function(field, value) {
    var newForm = {};
    Object.keys(serviceForm).forEach(function(k) { newForm[k] = serviceForm[k]; });
    newForm[field] = value;
    setServiceForm(newForm);
  };

  var handleUpdateConfig = function() {
    axios.put('/api/config/config', configForm).then(function() {
  ...
  var handleDeleteBox = function(box) {
    if (window.confirm('Supprimer ' + box.box_number + ' ?')) {
      axios.delete('/api/config/boxes/' + box.id).then(function() { fetchData(); });
    }
  };
  ...
  var handleDeleteService = function(service) {
    if (window.confirm('Supprimer ' + service.service_name + ' ?')) {
      axios.delete('/api/config/services/' + service.id).then(function() { fetchData(); });
    }
  };
        ...
        <div>
          <div style={s.grid3}>
            <div style={s.statCard}>
              <div style={s.statValue}>{boxes.length}</div>
              <div style={s.statLabel}>📦 Boxes</div>
            </div>
            <div style={s.statCard}>
              <div style={s.statValue}>{services.length}</div>
              <div style={s.statLabel}>🛎️ Services</div>
            </div>
            <div style={s.statCard}>
              <div style={s.statValue}>{configForm.tax_rate ? (configForm.tax_rate * 100).toFixed(0) : 0}%</div>
              <div style={s.statLabel}>💶 TVA</div>
            </div>
          </div>
          ...
          <div style={Object.assign({}, s.card, { marginTop: 24 })}>
            ...
            <div style={{ marginTop: 20 }}>
              <div style={s.grid2}>
                <div>
                  <label style={s.label}>Nom de la pension</label>
                  <input style={s.input} value={configForm.pension_name || ''} onChange={function(e) { updateConfig('pension_name', e.target.value); }} />
                </div>
                <div>
                  <label style={s.label}>Téléphone</label>
                  <input style={s.input} value={configForm.phone || ''} onChange={function(e) { updateConfig('phone', e.target.value); }} />
                </div>
              </div>

              <div style={{ marginTop: 16 }}>
                <label style={s.label}>Adresse</label>
                <input style={s.input} value={configForm.address || ''} onChange={function(e) { updateConfig('address', e.target.value); }} />
              </div>

              <div style={{ ...s.grid2, marginTop: 16 }}>
                <div>
                  <label style={s.label}>Nombre total de boxes</label>
                  <input style={s.input} type="number" value={configForm.total_boxes || ''} onChange={function(e) { updateConfig('total_boxes', e.target.value); }} />
                </div>
                <div>
                  <label style={s.label}>TVA (%)</label>
                  <input style={s.input} type="number" step="0.01" value={configForm.tax_rate ? (configForm.tax_rate * 100).toFixed(2) : 0} onChange={function(e) {
              ...
              <div style={{ marginTop: 16 }}>
                ...
                <input
                  ...
                  type="number"
                  step="0.01"
                  value={configForm.default_daily_rate !== undefined && configForm.default_daily_rate !== null ? configForm.default_daily_rate : ''}
                  onChange={function(e) { updateConfig('default_daily_rate', e.target.value); }}
                  placeholder="30.00"
                />
              </div>
        ...
        <div>
            ...
            <div style={s.card}>
              ...
              <div style={{ ...s.grid2, marginTop: 20 }}>
                <div>
                  ...
                  <input style={s.input} value={boxForm.box_number} onChange={function(e) { updateBox('box_number', e.target.value); }} placeholder="A-01" />
                </div>
                <div>
                  <label style={s.label}>Type</label>
                  <select style={s.select} value={boxForm.box_type} onChange={function(e) { updateBox('box_type', e.target.value); }}>
                ...
                <div>
                  ...
                  <input style={s.input} type="number" value={boxForm.capacity} onChange={function(e) { updateBox('capacity', e.target.value); }} />
                </div>
                <div>
                  <label style={s.label}>Tarif/jour</label>
                  <input style={s.input} type="number" step="0.01" value={boxForm.daily_rate} onChange={function(e) { updateBox('daily_rate', e.target.value); }} />
                </div>
              </div>
          ...
          {boxMode === 'batch' && (
            <div style={s.card}>
              ...
              <p style={{ color: '#94a3b8', fontSize: 13 }}>Créer plusieurs boxes identiques en une fois</p>
              <div style={{ ...s.grid2, marginTop: 20 }}>
                <div>
                  <label style={s.label}>Préfixe</label>
                  <input style={s.input} value={batchForm.prefix} onChange={function(e) { updateBatch('prefix', e.target.value); }} placeholder="A" />
                </div>
                <div>
                  <label style={s.label}>Séparateur</label>
                  <select style={s.select} value={batchForm.separator} onChange={function(e) { updateBatch('separator', e.target.value); }}>
                    <option value="-">Tiret (A-01)</option>
                    <option value=" ">Espace (A 01)</option>
                    <option value="">Rien (A01)</option>
                  </select>
                </div>
                <div>
                  <label style={s.label}>Numéro de départ</label>
                  <input style={s.input} type="number" value={batchForm.startNum} onChange={function(e) { updateBatch('startNum', e.target.value); }} />
                </div>
                <div>
                  <label style={s.label}>Nombre de boxes</label>
                  <input style={s.input} type="number" value={batchForm.count} onChange={function(e) { updateBatch('count', e.target.value); }} />
                </div>
                <div>
                  <label style={s.label}>Type</label>
                  <select style={s.select} value={batchForm.box_type} onChange={function(e) { updateBatch('box_type', e.target.value); }}>
                    <option value="petit">Petit</option>
                    <option value="standard">Standard</option>
                    <option value="grand">Grand</option>
                  </select>
                ...
                <div>
                  <label style={s.label}>Capacité</label>
                  <input style={s.input} type="number" value={batchForm.capacity} onChange={function(e) { updateBatch('capacity', e.target.value); }} />
                </div>
                <div>
                  <label style={s.label}>Tarif journalier</label>
                  <input style={s.input} type="number" step="0.01" value={batchForm.daily_rate} onChange={function(e) { updateBatch('daily_rate', e.target.value); }} />
                </div>
              </div>
          ...
          <div style={s.card}>
            <h3 style={{ marginTop: 0 }}>📦 Liste des boxes ({boxes.length})</h3>
            <div style={{ marginTop: 20 }}>
          ...
          <div style={s.card}>
            ...
            <div style={{ ...s.grid2, marginTop: 20 }}>
              <div>
                ...
                <input style={s.input} value={serviceForm.service_name} onChange={function(e) { updateService('service_name', e.target.value); }} />
              </div>
              <div>
                <label style={s.label}>Type</label>
                <select style={s.select} value={serviceForm.service_type} onChange={function(e) { updateService('service_type', e.target.value); }}>
                  <option value="">--</option>
              ...
              <div>
                ...
                <input style={s.input} type="number" step="0.01" value={serviceForm.price} onChange={function(e) { updateService('price', e.target.value); }} />
              </div>
              <div>
                <label style={s.label}>Description</label>
                <input style={s.input} value={serviceForm.description} onChange={function(e) { updateService('description', e.target.value); }} />
              </div>
            </div>
